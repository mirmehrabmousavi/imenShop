...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            handleAddedTodo3(item){
                if(this.taxRoute == 'برند'){
                    this.i = 0;
                    for ( this.i ; this.i < item[0].length; this.i++) {
                        this.allTaxes.push(item[0][this.i].name);
                        this.taxId.push(item[0][this.i].id);
                        this.$emit('sendTax' , this.taxId);
                    }
                }
                if(this.taxRoute == 'دسته بندی'){
                    this.i = 0;
                    for ( this.i ; this.i < item[1].length; this.i++) {
                        this.allTaxes.push(item[1][this.i].name);
                        this.taxId.push(item[1][this.i].id);
                        this.$emit('sendTax' , this.taxId);
                    }
                }
                if(this.taxRoute == 'گارانتی'){
                    this.i = 0;
                    for ( this.i ; this.i < item[2].length; this.i++) {
                        this.allTaxes.push(item[2][this.i].name);
                        this.taxId.push(item[2][this.i].id);
                        this.$emit('sendTax' , this.taxId);
                    }
                }
                if(this.taxRoute == 'بازه زمانی'){
                    this.i = 0;
                    for ( this.i ; this.i < item[3].length; this.i++) {
                        this.allTaxes.push(item[3][this.i].name);
                        this.taxId.push(item[3][this.i].id);
                        this.$emit('sendTax' , this.taxId);
                    }
                }
            },
        ...
        created: function() {
            ...
            this.$eventHub.on('sendAllTaxes', this.handleAddedTodo3);
        },
        mounted() {
            this.getTax();
        }
